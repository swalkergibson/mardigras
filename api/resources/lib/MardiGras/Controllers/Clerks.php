...
class Clerks extends \MardiGras\Lib\MGControllerClass
{
	...
	public function addClerk($clerkGroupId, $code, $name, $password, $passwordConfirm)
	{
		...
		$group = $clerkGroups->get($clerkGroupId);
	...
	public function searchClerks()
	{
		$dql = 'select c from Entities\Clerks c where c.deleted != 1';
		$query = $this->em->createQuery($dql);
		$result = $query->getResult();
